...
public class CheckEndpointTests : IClassFixture<TestApp>
{
    ...
    public async Task Twenty_first_check_in_a_day_is_429()
    {
        ...
        Assert.Equal("That's today's 20 free checks. Pro gives you 100000 a month, and the stylist sees your whole wardrobe. Or come back tomorrow.", (await response.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("error").GetString());
