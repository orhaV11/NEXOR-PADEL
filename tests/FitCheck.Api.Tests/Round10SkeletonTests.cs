...
public class Round10SeamTests
{
    ...
    public async Task A_board_rank_notification_carries_its_rank_and_the_push_line_reads_it()
    {
        ...
        // The tap lands on the week that closed: an instant six and a half days before the send (mid-week whatever the DST
        // arithmetic of that week), which ?week= takes as an instant.
        var job = new PushJob(Guid.NewGuid(), NotificationType.BoardRank, "someone", null, null, null, 7);
        Assert.Equal("/#/board?week=2027-01-13T22:00:00Z", PushSender.UrlFor(job, new DateTime(2027, 1, 20, 10, 0, 0, DateTimeKind.Utc)));
        ...
        Assert.InRange(instant, DateTime.UtcNow.AddHours(-156).AddMinutes(-1), DateTime.UtcNow.AddHours(-156).AddMinutes(1));
